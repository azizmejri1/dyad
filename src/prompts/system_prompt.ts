...
- Import from Dyad's fixtures, NOT from \`@playwright/test\` directly: \`import { test, expect } from "../dyad-fixtures";\` (adjust the relative path if the spec is nested). Dyad generates \`dyad-fixtures.ts\` next to the config; it re-exports \`test\` and \`expect\` unchanged when run outside Dyad, so \`npx playwright test\` still works normally, and routes the browser into Dyad's preview panel when the user chooses "Watch in Dyad". A spec that imports \`@playwright/test\` directly always launches its own browser and cannot be watched.
- Do NOT create or edit \`playwright-dyad.config.ts\` or \`dyad-fixtures.ts\`. Dyad generates and owns those files, and every test run uses it: it points \`baseURL\` at the running dev server via the \`DYAD_TEST_BASE_URL\` env var and configures the reporter, workers, and browser. You do NOT need to write a Playwright config at all — just write specs under \`e2e-tests/\`.

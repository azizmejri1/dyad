...
export interface RunTestsWithIsolationOptions {
   ...
   * opens the CDP endpoint at boot) and the preview showing this app.
   */
  preview?: boolean;
}
        ...
        if (previewWindow && previewCdpEndpoint) {
          ...
          if (!ready.ok) {
            return {
              ...
              results: [],
              infraError: {
                message: `The preview panel isn't showing this app (${ready.reason}). Open the Preview tab with the native preview enabled, then try again.`,
              },
              isolation: prepared.isolation,
            };
          }

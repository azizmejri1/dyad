 ...
 */
export const PREVIEW_SHIM_RELATIVE_PATH = `${E2E_TEST_DIR}/fixtures/dyad-test.ts`;
 ...
 * here, so unmodified specs pick this up. The shim re-exports the real runner
 * from `playwright/test` — a genuine entrypoint of the `playwright` package
 * that `@playwright/test` depends on — which sidesteps the alias pointing at
 * itself.
 *
  ...
  return `${JSON.stringify(
    {
      ...
      compilerOptions: {
        baseUrl: ".",
        paths: {
          "@playwright/test": ["./fixtures/dyad-test.ts"],
        },
      },
    },
    null,

...
/**
 * Playwright's `use.screenshot` setting. `"on"` captures a screenshot at the
 * end of every test — what the before/after UI screenshots feature needs — and
 * is only used when the user has turned that setting on. Everything else runs
 * with the failure-only default so no extra artifacts are written.
 */
export type ScreenshotMode = "on" | "only-on-failure";
...
export function buildPlaywrightConfig(
  ...
  screenshotMode: ScreenshotMode = DEFAULT_SCREENSHOT_MODE,
): string {
...
function writePlaywrightConfig(
  ...
  fs.writeFileSync(
    ...
    buildPlaywrightConfig(channel, screenshotMode),
  );
  logger.info(
    `Wrote ${DYAD_CONFIG_FILENAME} (${channel ? `channel: ${channel}` : "bundled chromium"}, screenshot: ${screenshotMode})`,
  );
}

/**
 * Point a Dyad-generated config's `use.screenshot` at `mode`.
 *
 * Runs on every bootstrap so the config tracks the current setting in both
 * directions: turning the before/after screenshots setting on upgrades existing
 * apps to `"on"`, and turning it back off returns them to failure-only capture
 * (the guarantee that no screenshots are taken while the setting is off).
 * Only touches configs carrying the sentinel, so a config the user has adopted
 * is never rewritten, and only the one line changes so a `channel:` already in
 * the file survives.
 */
function syncConfigScreenshotMode(appPath: string, mode: ScreenshotMode): void {
  if (!isDyadGeneratedConfig(appPath)) return;
  const text = readDyadConfigText(appPath);
  if (!text) return;
  const current = /screenshot:\s*"(on|only-on-failure)"/.exec(text);
  if (!current || current[1] === mode) return;
  fs.writeFileSync(
    dyadConfigPath(appPath),
    text.replace(current[0], `screenshot: "${mode}"`),
  );
  logger.info(`Migrated ${DYAD_CONFIG_FILENAME} screenshot mode to ${mode}`);
}
...
export async function ensurePlaywrightBootstrap({
  ...
  signal,
  onOutput,
  screenshotMode = DEFAULT_SCREENSHOT_MODE,
}: {
  appPath: string;
  signal?: AbortSignal;
  onOutput?: (chunk: string) => void;
  /** `use.screenshot` the generated config should carry for this run. */
  screenshotMode?: ScreenshotMode;
  ...
  if (!hasDyadPlaywrightConfig(appPath)) {
    ...
    if (!usesChannel && detectedChannel && isDyadGeneratedConfig(appPath)) {
      writePlaywrightConfig(appPath, detectedChannel, screenshotMode);
      usesChannel = true;
      onOutput?.(
  ...
  // Keep `use.screenshot` in step with the user's before/after screenshots
  // setting (both directions — see syncConfigScreenshotMode).
  syncConfigScreenshotMode(appPath, screenshotMode);
